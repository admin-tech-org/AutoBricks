...
不用眼睛仿、不抄原始碼——叫瀏覽器把原站每個元素「量」出精確數值、存成檔案，
之後蓋 Bricks、驗收，全部對著這份數值檔做。同一顆按鈕，三種做法的差別：
...
工具就是瀏覽器自己的 API：`getComputedStyle`（層疊算完的最終樣式）、`getBoundingClientRect`
（精確盒尺寸）、`getAnimations`（動畫清冊）。這份檔案＝下面名詞裡的 **dump**。

**為什麼要這樣**：因為**數字可以機器對帳**。複刻品推上 WP 渲染後，用同一套方法把渲染結果
也量一次——`渲染出來的值 − 當初量到的值 ＝ 差異清單`，差幾 px 就修幾 px，減到零（或門檻內）
才算完成（＝帳③）。「像不像」從主觀感覺變成一道**可以逐項打勾的減法**——不准憑感覺猜。

**附帶陷阱**：量到的是「用值」不一定是「設計值」（`line-height` 量到 `24px`，作者其實寫
`1.5`；照抄 24px 大標行高就壞）——量完還要判斷數字背後的設計意圖，不能無腦照抄（詳見鐵律 2）。
...
>   （批次 id＝Phase 0 第 7 條的 `<年月日-時分秒-站名>`）。Phase 1 建立、之後全程讀寫：
>   區清單與各區狀態（pending→done）、各區四本帳 ledger、page_behaviors、tokens、資產清單、
>   env_baseline（健檢基準）、band_pages（分身頁號）、pitfalls（踩過的坑）。
>   host 與所有工人共用；**多工人同時在跑時，寫它一律「重讀最新→只改自己那區→原子寫回」**
>   ——併發直寫實測會撞出壞 JSON。
...
- **這張表怎麼讀——「帳」是四個要結清的問題，結帳靠兩份證據檔對答案**：
  **dump**（機器量出來的數值檔）與**視覺清單**（原站截圖逐項列成的文字清單）。對照關係：
  帳① 兩份都用（DOM 走訪對 dump 驗「每個節點都有決定」、對視覺清單驗「看得到的都有節點承接」）；
  帳③ 用 dump（渲染值 − dump 值，減到清空）；帳④ 用視覺清單（逐項核渲染截圖）；
  帳② 用自己的行為清冊＋真滑鼠實測。**同一份證據檔可以服務多本帳**（dump 服務①③、
  視覺清單服務①④）——所以「視覺」不是第五本帳，是 ①④ 共用的證據；
  帳④ 真正獨佔的題目是「合成後的整體長相」（合成／疊層／接縫這類逐項比對蓋不到的）。
- **帳② 是「逐筆銷帳的清單」，鎖死動態的兩種死法**：**漏做**（原站有、你沒做——列上清冊的
  必須逐筆銷掉）與**腦補**（原站沒有、你憑「這種元件通常會⋯」補上——不在清冊上的不准做，鐵律 3）。
  條目怎麼來（對應三軸，見下）：CSS 狀態與動畫**機器列舉得到**（七態掃描、`getAnimations()`——
  不用互動就拿得到）；**JS 監聽器沒有列舉法**，只能對候選名單（`cursor:pointer`、原生互動元素、
  動畫目標）真滑鼠逐個探，且「探了哪些」要記下來、別假裝完整。
  **發現靠列舉，定罪靠互動**：`forcePseudoState` 只證明「有宣告」、不證明「真的會動」——
  每筆要標 `verified` 一律真滑鼠＋等轉場走完；宣告了但戳不動＝死碼 `excluded`＋理由。
  順序照鐵律 4：先捲（看什麼**自己**在動）→ 再機器列舉 → 最後互動驗證。
...
- 帳④ 為什麼用門檻不用 0：**0 分不出「做錯了」和「環境不一樣」**。真的錯（位移、少東西、顏色錯）
  ...
  **門檻由 host 設一次（預設：色版差 ≤8 不算差異），band 工人拿到的仍是「過／不過」；
  超過門檻的一律查到原因，不准把門檻調高來遮。**
- **帳① 與帳④ 共用同一份「視覺清單」，一前一後雙把關**：處置動手前，先把原站該區截圖
  **用文字逐項列成清單**（底色／背景圖案／疊層／裝飾／每一個元件），拿清單跟 DOM 處置對帳——
  **DOM 走訪保證「樹上每個節點都有決定」，視覺清單保證「畫面上看得到的都有節點承接」**：
  漏建的元件在開工前就現形，不是等收帳才發現。收帳時再拿**同一份清單**核渲染截圖。
  **前段＝DOM 樹×視覺雙把關（帳①），後段＝diff×視覺雙把關（帳③④）。**
...
- **主頁整合**：收帳滿一小批（2–3 區）就把整頁重推主 PAGE_ID 一次＋**銜接抽驗**——接縫、
  ...
  （上半＝前區的尾、下半＝後區的頭），每條縫一張——**不拍整頁高圖**（又慢又縮糊，
  見 Phase 0 第 5 條）；分身頁 0-diff 不保證整合後零新干擾。
- 驗收照舊四面向（工件已落檔，讀檔不搶瀏覽器；動態抽驗用主台或任何空台）。
- 跨區一致性優化、RWD 整頁總檢、收尾。
- **順序解禁**：分身頁各區 y 獨立，工人不必由上到下；主頁整合仍照文件序組裝。

**★Monitor 看門狗——凡有派出去的工人就開（含 Phase 1 探索工人，不限併行；實測最嚴重的卡死
就發生在單工人的 Phase 1）**：host 用 `Monitor` 背景腳本盯**各工人的產物與暫存活動**
（`data/<id>/dumps/<區>/`、`template-<區>.json`、`tmp/` 裡帶該區名的 scratch——工人量測階段
常只寫 scratch、甚至 20–30 分什麼都不落檔）——某工人**超過 ~30 分完全沒有新檔**＝疑似卡在
一個不返回的呼叫（**別設 10 分：正常量測本來就安靜 20–30 分，會連環誤報**——實測調到 30 分才止住）。
**★重點：不只 fullPage——任何 browser 操作都可能 hang**（CDP 抽風、頁面 wedged、網路卡住，
或某個還沒遇過的操作）。**看門狗的最大用意正是這個：它不需要知道是哪個操作卡住、也不需要
事先列出所有會 hang 的動作，只要偵測到「這個工人不再產出」就能兜住——連預料外的卡死都抓得到。**
**工人卡死時自己救不了自己**（它在等那台塞死的瀏覽器），只有 host 能救：Monitor 通知後，host 先
`SendMessage` 問一句確認（排除「正在跑正當的長量測」）、再看瀏覽器——確認卡死就**重啟那一台
瀏覽器**（同一支啟動腳本、埠當參數，斷掉卡死的連線）→ 叫工人重新決策或重派。
**逐台獨立：一台卡死只重那台，其餘不動。**
...
所以整合主頁、批次銜接抽驗之後、收尾之前，**必派一個全新乾淨 context 的稽核工人**：
...
5. **⚠️ 整頁高圖（fullPage／CDP `captureBeyondViewport`）不在流程中使用**——高頁上會 hang
   或跑數十分鐘、塞死整條 MCP 連線（實測兩次：80 分／45 分），且高圖給模型看會被縮到
   細節全糊、沒有驗收價值。**截圖一律視口大小**：基準圖＝一屏一串（Phase 1 第 1 步）、
   接縫＝跨縫視口圖（見「一之二」銜接抽驗）、**區高於視口＝該區也分張**（2–3 張，
   不硬拍單張長元素圖——那是高圖問題的縮小版）。真有理由截 fullPage（不建議）：它還會
   **永久弄髒分頁**（捲軸被藏、內容區寬度改變）——量測全做完才截、截完該分頁報廢換新；
   卡住由 Monitor 看門狗兜。
...
**動手前先建「視覺清單」**（原站該區截圖→文字逐項列：底色／圖案／疊層／裝飾／每個元件），
處置完逐項對帳——每個視覺項都要對到 build／repeat 的節點，skip／collapse 掉任何看得到的東西
必須附理由（見四本帳「帳①④ 共用視覺清單」）。這份清單就是帳④ 收帳用的同一份。
...
- **帳④ 視覺把關**：拿**處置階段（③）建好的那份視覺清單**（同一份清單一前一後雙把關，
  見四本帳）→ 渲染截圖逐項核。**外加機器覆蓋核對**：對渲染頁該區跑 toolkit 的覆蓋普查
