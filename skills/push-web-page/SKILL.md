...
1. **素材先上傳。** Agent 產品透過 WP File Manager 進入目標站實際的 uploads 目錄（通常為 `wp-content/uploads/`），優先使用「上傳資料夾／Select Folder」選取部署副本的 `assets/<asset-folder>/`，保留素材子資料夾、內部結構與檔名。介面不支援或資料夾上傳失敗時，Agent 產品再評估於 `tmp/` 建立只含素材資料夾的暫存 ZIP，透過介面上傳及解壓，確認後移除伺服器上的暫存 ZIP。Agent 產品不將整份交付包、PHP 片段、部署腳本或驗收資料上傳至 uploads，也不改走媒體庫另建年月目錄。Agent 產品核對實際落點、檔案完整性、公開網址與包內引用一致，且代表素材能載入，再繼續匯入。
...
3. **匯入模板並套用頁面。** Agent 產品透過 Bricks 正常匯入部署副本的 `template.json`，將模板及 `pageSettings` 套用至本次頁面並儲存，以 JSON 原有內容與設定為準。若頁面設定未隨模板套用，Agent 產品透過介面補套原值，不自行改寫 Body classes 或決定頁首頁尾的顯示方式。Agent 產品核對素材引用與頁面識別標記均已生效。整包交付 ZIP 不是 Bricks 模板匯入檔，模板出現在模板庫也不代表頁面已完成。圖片下載、SVG 等匯入選項以目標版本實測為準，不直接套用其他環境的經驗。

Agent 產品在每次上傳、匯入、啟用及儲存後檢查結果，再進行下一項。中斷或逾時後先查看後台現況，沿用已完成項目，不盲目重送。若匯入會覆蓋既有全域樣式或其他頁面的資料，Agent 產品先保留現況並釐清影響，不為完成單頁部署改動無關頁面。

## 驗收與回報

- Agent 產品確認儲存完成後，重整或重新開啟 Bricks 編輯器，等待內容載入，再核對元素與頁面設定是否符合部署副本的 `template.json`，並檢查代表文字、圖片及容器可編輯。
- Agent 產品另開分頁檢視目標頁面的前台預覽，以實際頁面驗收外觀與互動，不只依據編輯器中央的預覽畫面。Agent 產品依交付包的 `report.md`、`comparison.html` 及代表截圖，比對桌面與手機版型、關鍵斷點、主要互動與動畫，實際檢視截圖及素材，並確認 PHP 片段生效、素材不依賴來源測試站或登入狀態。已授權發佈的頁面另確認未登入的訪客可正常使用。
