...
驗證器檢查元素 ID、父子引用、部分設定及 `pageSettings` 資料格式。提供 live schema 時，另核對元素與頁面設定的候選欄位，未知設定會警告，缺少對應 schema 時明示略過。原始碼掃描會合併可辨識的父類別欄位，但不執行 PHP，動態組裝、條件與覆寫的結果仍需配合 theme 原始碼與實測判讀。成功保存設定不代表 Bricks 已採用該欄位。
