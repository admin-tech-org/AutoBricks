...
| `.codex/config.toml` | Codex 讀取本專案是否啟用已安裝 plugin 的設定 |
...
上述 `enabled = true` 是啟用安裝版本的範例。本 repo 目前設定為 `enabled = false`，讓開發時只使用專案內的 `.agents/skills/`；已安裝的 plugin 快照仍保留，使用者需要測試安裝版本時再啟用。
...
本次未提交版本以 Git 可追蹤檔案的乾淨快照安裝，來源放在使用者本機快取目錄，原始 repo 維持單一份結構。`codex plugin marketplace list` 可查看目前來源。
...
此配置供 Git／本機 marketplace 發行，尚未提交 OpenAI 公開 Plugin Directory。plugin-creator 的靜態驗證器限定技能目錄為 `skills/`，無法驗證本 repo 在 manifest 指定的 `.agents/skills/` 路徑；該路徑已以 Codex CLI 安裝與技能探索確認相容。本次技能內容更新未重新安裝，先前的快照仍保留並在開發專案停用。格式依據：[OpenAI plugin 包裝文件](https://developers.openai.com/plugins/build/plugins)。
