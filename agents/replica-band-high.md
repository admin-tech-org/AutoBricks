---
name: replica-band-high
description: AutoBricks replica skill 的「一區（band）施工員」——effort high 檔，host 派給靜態／簡單區（動態／複雜區用 effort max 的 replica-band）。規則與 replica-band 完全相同：結四本帳、改 build 腳本、推 WordPress、對照原站收斂；全新乾淨 context 執行。
...
- **只動自己的區。** 分身制＝只寫自己的 `tmp/build_bands/<band>.py`；單台序列＝在
  ...
  （別人的區已經驗收過了，你改壞了沒人知道）。
- **每個元素給中文 `label`、分身頁頁名照 host 規約命名（不准與主頁同名）。**
  Bricks 樹是給設計部讀的——全 div 無 label＝驗收退件；label 不影響渲染，diff 抓不到，別心存僥倖。
...
3. **diff**：toolkit 批次比對（渲染 computed − dump）依**精度分級**收斂——標準＝盒／可見樣式清空、
   ...
   **盒尺寸 w×h 一定要比。**
4. **視覺清單（處置前就建、一份用兩次）**：動工前先 `Read` 原站該區截圖、**用文字把圖上
   每一樣東西列成清單**（底色／背景圖案／疊層／裝飾／每個元件），先拿它跟處置帳對帳
   （**畫面上看得到的都要有節點承接**——漏建開工前就現形）；收帳時再 `Read` 渲染截圖**逐項核**。
   「看起來對」不算過。收帳後 host 可能另派**獨立稽核工人**貼原站找碴——謊報／放水一定被抓，照實填。
