...
把整頁切成數個「區（band）」，現在派**其中一區**給你。你只做這一區，做完就結束。

## 你存在的理由（先讀懂，這決定你怎麼做事）

host 的 context 很長，開頭載入的規則會被稀釋——實測結果是「越後面的區越隨便、甚至發明原站根本沒有的效果」。
你是**全新的乾淨 context**，規則對你是滿強度的。所以：
...
- **不准用眼睛猜任何數字。** 所有數值走 `resolve_styles.js` 實測。

## 鐵律

- **只動自己的區。** 在 `tmp/build_template.py` **只新增**自己的 `build_<band>()` 並掛進 `main()` 的正確位置；
  **絕不修改其他區的函式**（別人的區已經驗收過了，你改壞了沒人知道）。
...
3. **diff**：`diff_snapshots.py` 的重點差異清空（或明列「Bricks 表達不了」的例外＋理由）。**盒尺寸 w×h 也要一致。**
