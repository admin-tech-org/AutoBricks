...
#  真 Chrome + 真 profile = 沒有自動化指紋，防爬蟲較嚴的網站也能正常渲染。
#  目標網站若需要登入，在這個視窗登一次即可——登入態存在同層的 .chrome_cdp
#  （已 gitignore），下次免再登。
#  ── 這是範本（macOS / Linux 用）：setup 會把它複製到「使用者專案」底下的 .browser/。
# ===================================================================
set -euo pipefail
...
if [ -z "$CHROME" ]; then
  echo "[x] 找不到 Chrome/Chromium，請編輯本檔把 CHROME 設成實際的 Chrome 路徑。"
  exit 1
fi
...
echo
echo " Next: 保持這台 Chrome 開著即可——Claude Code 的 Playwright MCP 用到時會自動接管 9222。"
