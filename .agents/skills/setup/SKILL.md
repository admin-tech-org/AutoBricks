...
- **Node（≥22）**：供共用的 `.codex/tools/browser.mjs` 使用。
...
### 5. 準備 Chrome CDP 連線
Agent 產品透過 Chrome CDP 觀察原站或 WP 預覽的內容、排版與互動，可使用現有的 CDP 工具或共用的 `<PLUGIN_ROOT>/.codex/tools/browser.mjs`。共用工具的連線方式見 `<PLUGIN_ROOT>/.codex/README.md`。啟動腳本的預設埠為 9222，Agent 產品需確認瀏覽器與所用工具設定的埠一致。
