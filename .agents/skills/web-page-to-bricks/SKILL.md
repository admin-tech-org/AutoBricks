...
- Agent 產品開始重建前，向使用者確認目標站的「WordPress 位址」與「網站位址」完整網址，臨時網域亦可。對話已有資料時直接沿用。
- Agent 產品另依後台或可正常載入的素材網址確認 uploads 的實際公開位置，不只憑兩個位址是否相同推定路徑。
- 使用者尚未指定部署站時，Agent 產品可先觀察原站，待確認目標站或僅做本機測試後再決定交付素材路徑。
...
- Agent 產品首次重建時，以 `ab-page-` 加當次任務資料夾名稱建立固定的頁面識別碼。Agent 產品將識別碼加入 Bricks 頁面的 Body classes，以及該頁每個 Code Snippets 片段的 tag，並讓片段只在帶此標記的頁面執行。搬站、改網址與續修保留識別碼，另做獨立頁面時才建立新識別碼。
- 頁面與片段的對應保存在 WP 後台，使後續 Agent 產品可從指定頁面的識別碼找到相同 tag 的片段，不依賴本機舊 `data/`。Agent 產品不將沒有相符標記的片段推定為該頁所有；歸屬不明時先釐清。
...
- Agent 產品將圖片、SVG、影片、字型及文件等非程式碼素材放在 `output/assets/<asset-folder>/`，交付模板與 Snippets 的素材引用需對應包內檔案，不能只依賴測試站專有的附件網址或 ID。使用者首次部署時，透過 File Manager、SSH／SFTP 等方式將 `<asset-folder>` 整個上傳至目標站的 uploads 目錄，通常為 `wp-content/uploads/`。素材資料夾使用清楚、穩定的名稱，不需跟著交付 ZIP 的時間戳變更。
- Agent 產品在模板與 Snippets 中，將同站素材寫成以 `/` 開頭的根相對網址，例如 `/wp-content/uploads/<asset-folder>/hero.jpg`。目標站的公開路徑含子目錄時一併保留。外站連結及不同網域的素材保留正確的完整網址，不直接移除網域。本機與目標站的素材路徑不同時，Agent 產品區分測試副本與交付副本，並在報告寫明目標素材路徑。只換網域且素材路徑不變時，不需替換同站素材網址。
...
- **格式與匯入**：檢查 JSON 驗證結果，確認正常匯入及編輯器儲存後的元素、CSS 選擇器、素材與腳本仍有效。根相對素材網址需在前台、編輯器及不同頁面路徑下正常載入，不依賴測試站網域。直接推送成功不等於正常匯入通過。
...
- **部署副本**：從最終 ZIP 解壓後測試素材資料夾更名腳本，檢查模板、Snippets 匯入檔、PHP 副本與實際素材路徑一致，版本檔及頁面識別碼保留，且原始包保留。正常匯入與瀏覽器驗收需對應包內的同一份模板及片段。
...
Agent 產品以 `<PLUGIN_ROOT>/output_schema_version` 為當次交付結構的版本依據，核對完成後原樣複製到 `output/`。該版本只描述交付結構，不代表 Bricks 版本，也不寫入 WP 頁面或片段。續修舊包時先檢查原版本，不只替換版本檔就視為升級。

Agent 產品提供能在解壓目錄以 `uv run` 執行的 `rename-assets-folder.py`，將實際指令寫入 `report.md`。腳本接受新的素材資料夾名稱，在部署副本重新命名 `assets/<asset-folder>/`，並依包內已確認的素材路徑同步更新模板、Snippets 匯入檔與 PHP 副本，不改外站連結。名稱不得造成目錄越界或覆蓋既有素材。

腳本不依賴 AutoBricks 原始碼或 `tmp/`，保留原始交付檔案、版本檔及頁面識別碼，不直接修改遠端 WP。Agent 產品在報告說明部署副本的用途，不以省略驗收證據的部署副本取代完整交付包。若目標站的 uploads 路徑改變，需先調整副本內的素材路徑再驗收，不能把更換網域視為搬動目錄。
...
- 參考網址、模板 JSON、可開啟的 WP 預覽網址，以及目標站的素材公開路徑與搬移所需檔案。
- 交付結構版本、頁面識別碼與片段 tag 的用途。使用者修改內容或程式碼時保留識別 class 與 tag，供後續對應。
